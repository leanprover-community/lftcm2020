...
-/
def top : subgp G :=
{ carrier := set.univ, -- this is just G, considered as a subset of G
  mul_mem := begin
    sorry
  end,
  one_mem := begin
...
instance : complete_lattice (subgp G) := complete_lattice_of_Inf _
begin
...
2) X = set, α := set (set X), β := topologies on X,
...
example : complete_lattice (subgp G) :=
galois_insertion.lift_complete_lattice gi_subgp
...
lemma subset_forget {X : Type} (Us : set (set X)) : Us ≤ forget (generate_from Us) := sorry

lemma generate_forget {X : Type} (τ : topological_space X) : generate_from (forget τ) = τ := sorry
